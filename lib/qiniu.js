...
function isImg (suffix) {
  let imgSuffix = ['png', 'jpg', 'jpeg', 'gif', 'bmp', 'webp', 'ico'];
  return imgSuffix.indexOf(suffix.toLowerCase()) > -1;
}
...
function processImgUrl(key, imgFile) {
  let config = getUserConfig();
  let processor = config.processor;
  let separator = config.separator;
  let domain = config.domain;
  let suffix = '';
  if (imgFile && processor && separator) {
    suffix = separator + processor;
  }
...
module.exports = {
  upload: function (filepath) {
    ...
    let matchs = filepath.split('/').pop().match(/[\/\\]?([^\.]+)\.*(.*)$/);
    let name = matchs[1];
    let suffix = matchs[2];
    let isImgFile = isImg(suffix);
    ...
    uploader.putFile(token, encodedName, filepath, extra, function (err, body, info) {
      ...
      if (!err && info.statusCode == 200) {
        let url = processImgUrl(body.key, isImgFile);
    ...
    manager.listPrefix(bucket, opt, (err, respBody, respInfo) => {
      ...
      if (respInfo.statusCode == 200) {
        respBody.items.forEach(item => {
          let suffix = item.key.split('.').pop();

          item.url = processImgUrl(item.key, isImg(suffix));
        })
        cb({
          list: respBody.items,
